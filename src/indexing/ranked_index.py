"""
Ranked inverted index with word count-based scoring
"""
import pickle
import time
import sys
...
class RankedIndex(BaseIndex):
    """Inverted index with term frequency-based ranking"""
    
    def __init__(self, version: str = "v1.0", index_name: str = "ranked_index",
                 use_stemming: bool = True, use_stopwords: bool = True):
        """
        Initialize ranked index
        ...
        # Inverted index: term -> {doc_id: (term_frequency, [positions])}
    ...
    def add_document(self, doc_id: str, text: str, metadata: Dict[str, Any] = None):
        ...
        for term, positions in term_positions.items():
            ...
            self.inverted_index[term][doc_id] = (term_freq, positions)
    ...
    def search(self, query: str, top_k: int = 10) -> List[Dict[str, Any]]:
        """
        Search using term frequency-based ranking
        ...
        for term in query_terms:
            if term in self.inverted_index:
                for doc_id, (term_freq, _) in self.inverted_index[term].items():
                    # Simple term frequency score
                    # Normalized by document length to avoid bias toward longer documents
                    doc_scores[doc_id] += term_freq / self.doc_lengths[doc_id]
